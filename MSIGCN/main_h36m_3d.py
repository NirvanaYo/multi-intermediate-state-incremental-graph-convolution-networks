import os
import sys
#sys.path.append('/mnt/hdd4T/mtz_home/code/wsPredictionRelease/')
sys.path.append(os.path.abspath('./'))
from utils import h36motion3d as datasets
from model import period_6
...
def main(opt):
    ...
    if not opt.is_eval:
        # dataset = datasets.Datasetsws(opt, split=0)
...
def ws(src,sample_len, kernel_size):
    att = torch.zeros(sample_len,sample_len).cuda()
    # w = torch.eye()
    for i in range(kernel_size,sample_len):
        for j in range(kernel_size,i+1):
            att[i, j] = j - kernel_size + 1
    for i in range(kernel_size, sample_len):
        att[i] = att[i] / torch.sum(att[i])
    d = torch.eye(sample_len).cuda()
    for i in range(kernel_size, sample_len):
        d[i][i] = torch.sum(att[i])
    d = torch.linalg.inv(d)
    d = torch.sqrt(d)
    rst = torch.matmul(d, att)
    rst = torch.matmul(rst, d)
    rst = torch.matmul(rst, src)
    rst[:,0:kernel_size] = src[:,0:kernel_size]
    return rst
def run_model(net_pred, optimizer=None, is_train=0, data_loader=None, epo=1, opt=None):
    ...
    for i, (p3d_h36) in enumerate(data_loader):
        ...
        p3d_sup_6 = p3d_h36.clone()[:, :, dim_used][:, -out_n - seq_in:].reshape(
            [-1, seq_in + out_n, len(dim_used) // 3, 3])
        p3d_sup_5 = ws1.clone()[:, -out_n - seq_in:].reshape(
            [-1, seq_in + out_n, len(dim_used) // 3, 3])
        p3d_sup_4 = ws2.clone()[:, -out_n - seq_in:].reshape(
            [-1, seq_in + out_n, len(dim_used) // 3, 3])
        p3d_sup_3 = ws3.clone()[:, -out_n - seq_in:].reshape(
            [-1, seq_in + out_n, len(dim_used) // 3, 3])
        p3d_sup_2 = ws4.clone()[:, -out_n - seq_in:].reshape(
            [-1, seq_in + out_n, len(dim_used) // 3, 3])
        p3d_sup_1 = ws5.clone()[:, -out_n - seq_in:].reshape(
            [-1, seq_in + out_n, len(dim_used) // 3, 3])


        # p3d_out_all_4, p3d_out_all_3, p3d_out_all_2, p3d_out_all_1 = net_pred(input, input_n=in_n, output_n=out_n, itera=itera)
        p3d_out_all_6, p3d_out_all_5, p3d_out_all_4, p3d_out_all_3, p3d_out_all_2, p3d_out_all_1 = net_pred(input, input_n=in_n, output_n=out_n, itera=itera)


        p3d_out_6 = p3d_h36.clone()[:, in_n:in_n + out_n]
        p3d_out_6[:, :, dim_used] = p3d_out_all_6[:, seq_in:]
        p3d_out_6[:, :, index_to_ignore] = p3d_out_6[:, :, index_to_equal]
        p3d_out_6 = p3d_out_6.reshape([-1, out_n, 32, 3])

        p3d_h36 = p3d_h36.reshape([-1, in_n + out_n, 32, 3])
        p3d_out_all_6 = p3d_out_all_6.reshape([batch_size, seq_in + out_n, len(dim_used) // 3, 3])
        p3d_out_all_5 = p3d_out_all_5.reshape([batch_size, seq_in + out_n, len(dim_used) // 3, 3])
        ...
        grad_norm = 0
        if is_train == 0:
            loss_p3d_6 = torch.mean(torch.norm(p3d_out_all_6 - p3d_sup_6, dim=3))
            loss_p3d_5 = torch.mean(torch.norm(p3d_out_all_5 - p3d_sup_5, dim=3))
            ...
            loss_all = (loss_p3d_6 + loss_p3d_5 + loss_p3d_4+ loss_p3d_3 + loss_p3d_2 + loss_p3d_1)/6
            optimizer.zero_grad()
            loss_all.backward()
            nn.utils.clip_grad_norm_(list(net_pred.parameters()), max_norm=opt.max_norm)
            optimizer.step()
            # update log values
            l_p3d += loss_p3d_6.cpu().data.numpy() * batch_size


        if is_train <= 1:  # if is validation or train simply output the overall mean error
            mpjpe_p3d_h36 = torch.mean(torch.norm(p3d_h36[:, in_n:in_n + out_n] - p3d_out_6, dim=3))
            m_p3d_h36 += mpjpe_p3d_h36.cpu().data.numpy() * batch_size
        else:
            mpjpe_p3d_h36 = torch.sum(torch.mean(torch.norm(p3d_h36[:, in_n:] - p3d_out_6, dim=3), dim=2), dim=0)
